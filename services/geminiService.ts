...
const App: React.FC = () => {
  ...
  const handleSendMessage = async (text: string, attachments: Attachment[]) => {
    ...
    try {
        ...
        // Budget for Flash is ~24k, Pro is ~32k. We'll set a safe high default if enabled.
        ...
        // Use the actual error message to help the user debug environment issues
        ...
        setMessages(prev => prev.map(msg => {
            if (msg.id === botMessageId) {
                return {
                    ...msg,
                    content: `**System Error:** ${errorText}\n\n*Diagnostic Hint: Check if API_KEY is set in Vercel Environment Variables.*`,
                    isThinking: false
                };
            }
            return msg;
